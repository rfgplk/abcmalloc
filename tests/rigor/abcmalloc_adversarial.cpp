...
{
  ++c.verifies;
  if ( !abctest::fp_check(ls.ptr[s], ls.sz[s], ls.key(s), ls.gen[s]) )
    c.note_error(ls.key(s), ls.gen[s], 0);
...
barrier_wait(barrier_t &b, u32 &my_sense)
{
  ...
  if ( b.count.add_fetch(1u, micron::memory_order_acq_rel) == b.n ) {
    ...
    while ( b.sense.get(micron::memory_order_acquire) != my_sense )
      __cpu_pause();
  }
}
...
burst_worker(burst_ctx *c)
{
  for ( u64 it = 0; it < c->ops; ++it ) {
    ...
    if ( op < 55u ) {
      if ( c->ls.ptr[s] == nullptr )
        radv_alloc(c->ls, s, abctest::sample_size_longtail(c->rng), c->cnt);
    } else {
      if ( c->ls.ptr[s] )
        radv_free(c->ls, s, c->cnt);
    }
  }
