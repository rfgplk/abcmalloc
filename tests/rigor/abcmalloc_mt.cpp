...
#include <micron/thread/thread_types/auto_thread.hpp>
...
{
  while ( s->ready.get(micron::memory_order_acquire) != 1u )
    __cpu_pause();
...
main(void)
{
  ...
  test_case("bidirectional cross-thread alloc + free");
  {
    ...
    auto worker = [](micron::atomic_token<u32> *p, mt_state *st) {
      while ( p->get(memory_order_acquire) != 1u )
        __cpu_pause();
      for ( usize i = 0; i < ROUNDS; ++i )
        abc::dealloc(st->ptrs[i]);
  ...
  test_case("musage() aggregates across thread arenas");
  {
    ...
    auto worker = [](agg_state *st, u32 widx) {
      for ( usize i = 0; i < K; ++i )
        st->blocks[widx][i] = abc::alloc(ALLOC_SIZE);
      st->ready.fetch_add(1u, memory_order_acq_rel);
      while ( st->go_free.get(memory_order_acquire) == 0u )
        __cpu_pause();
      for ( usize i = 0; i < K; ++i )
        abc::dealloc(st->blocks[widx][i]);
...

    {
      ...
      while ( s.ready.get(memory_order_acquire) != NUM_WORKERS )
        __cpu_pause();
